<template>
  <el-dialog v-model="dialogVisible" :close-on-click-modal="false" width="580px" append-to-body>
    <template #header>
      <div class="dialog-header">
        ...
        <el-link
          ...
          href="./template/班级学生模板.xlsx"
          download="班级学生模板.xlsx"
        >
          下载导入模板
        </el-link>
      </div>
...
import { requestClassStudentImport } from '@/api/basicManagement/classManagement'
...
function openDialog(classInfo) {
  formData.append('classId', classInfo.id)
  formData.append('capacity', classInfo.capacity)
  fileList.value = []
...
async function submitFileForm() {
  ...
  const result = await requestClassStudentImport(formData)
